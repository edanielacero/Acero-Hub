...
function OptionsEditor({ options, onChange }: {
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      {options.length === 0 && (
        <p className="text-[11px] text-slate-400 dark:text-zinc-500 text-center py-1">Agrega al menos una opción</p>
      )}
    </div>
  )
}
...
function VariableForm({ initial, sessionId, onSave, onClose, loading }: {
  initial?: Variable
  sessionId: string
  onSave: () => void
  onClose: () => void
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-5">
      <div>
        ...
        <input
          className={inp}
          placeholder="Ej. Sesión de mercado, Razón de entrada..."
          value={label}
      ...
      <label className="flex items-center gap-3 cursor-pointer min-h-[44px] px-1">
        <div className="relative">
          ...
          <div className={`absolute top-0.5 left-0.5 w-5 h-5 rounded-full transition-all duration-200 ${
            isRequired ? 'bg-white translate-x-5' : 'bg-slate-400 dark:bg-zinc-500'
          }`} />
        </div>
        <div>
          <p className="text-[13px] font-medium text-slate-700 dark:text-zinc-300">Obligatorio</p>
          <p className="text-[11px] text-slate-400 dark:text-zinc-500">Se pedirá siempre al registrar un trade</p>
        </div>
      </label>

      {err && (
        <p className="text-[12px] text-red-500 dark:text-red-400 px-1">{err}</p>
      )}
    ...
    <div className={`flex items-center gap-3 px-4 py-3 rounded-2xl border transition-all duration-150 ${
      ...
      <div className="flex flex-col gap-0.5 shrink-0">
        <button
          ...
          disabled={index === 0}
          aria-label="Subir"
          className="w-7 h-7 flex items-center justify-center rounded-lg text-slate-400 dark:text-zinc-500 hover:text-slate-700 dark:hover:text-zinc-200 hover:bg-slate-100 dark:hover:bg-zinc-800 disabled:opacity-20 disabled:cursor-not-allowed transition-colors duration-150 cursor-pointer">
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 mb-0.5 flex-wrap">
          ...
          {variable.is_preset && (
            <span className="inline-flex items-center gap-1 text-[9px] font-black uppercase tracking-wider text-slate-400 dark:text-zinc-500">
              <IconLock size={9} />
              Preset
            </span>
          )}
        ...
        <span className="text-[11px] text-slate-400 dark:text-zinc-500 font-medium">{TYPE_LABELS[variable.type]}</span>
      ...
      <div className="flex items-center gap-1 shrink-0">
        ...
        <button
          ...
          aria-label={variable.is_active ? 'Desactivar' : 'Activar'}
          className="relative min-w-[44px] min-h-[44px] flex items-center justify-center cursor-pointer">
          <div className={`w-9 h-5 rounded-full transition-colors duration-200 border ${
            variable.is_active
              ? 'accent-toggle-on'
              : 'bg-slate-200 dark:bg-zinc-800 border-slate-300 dark:border-zinc-700'
          }`} />
          <div className={`absolute w-[15px] h-[15px] rounded-full transition-all duration-200 ${
            variable.is_active
              ? 'bg-white translate-x-[9px]'
              : 'bg-slate-400 dark:bg-zinc-500 -translate-x-[3px]'
          }`} />
        </button>
        ...
        <button
          onClick={onEdit}
          aria-label="Editar"
          className="min-w-[36px] min-h-[36px] flex items-center justify-center rounded-xl text-slate-400 dark:text-zinc-500 hover:text-slate-700 dark:hover:text-zinc-200 hover:bg-slate-100 dark:hover:bg-zinc-800 transition-colors duration-150 cursor-pointer">
          <IconEdit size={14} />
        </button>

        {/* Delete — only for custom variables */}
        {!variable.is_preset && (
          <button
            onClick={onDelete}
            aria-label="Eliminar"
            className="min-w-[36px] min-h-[36px] flex items-center justify-center rounded-xl text-slate-400 dark:text-zinc-500 hover:text-red-500 dark:hover:text-red-400 hover:bg-red-50 dark:hover:bg-red-500/10 transition-colors duration-150 cursor-pointer">
            <IconTrash size={14} />
          </button>
        )}
      </div>
...
function EmptyState({ onAdd }: { onAdd: () => void }) {
  return (
    <div className="flex flex-col items-center justify-center py-16 px-6 text-center">
      ...
      <p className="text-[13px] text-slate-400 dark:text-zinc-500 mb-6 leading-relaxed max-w-[220px]">
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-[#080808]">
      ...
      <div className="max-w-lg mx-auto px-4 pt-4 pb-2">
        <div className="flex items-center justify-between mb-1">
          <div>
            <h2 className="text-[18px] font-black text-slate-900 dark:text-white tracking-tight">Variables</h2>
            {!loading && (
          ...
          {variables.length > 0 && (
            <button
              ...
              className="flex items-center gap-2 px-4 min-h-[40px] rounded-xl text-[13px] font-bold accent-btn accent-btn-shadow transition-all duration-150 cursor-pointer">
              <IconPlus size={13} />
              Nueva
            </button>
          )}
      ...
      <div className="max-w-lg mx-auto px-4 pb-12">
        {loading ? (
          ...
          <div className="flex flex-col gap-5 pt-2">
            ...
            {presets.length > 0 && (
              <div>
                <p className="text-[9px] font-black uppercase tracking-[0.2em] text-slate-400 dark:text-zinc-500 mb-2 px-1">
            ...
            {reordering && (
              <p className="text-center text-[11px] text-slate-400 dark:text-zinc-500 animate-pulse">Guardando orden...</p>
            )}
          </div>
        )}
      </div>
      ...
      {deleteVar && (
        <DeleteSheet
          variable={deleteVar}
          onConfirm={() => deleteVariable(deleteVar)}
