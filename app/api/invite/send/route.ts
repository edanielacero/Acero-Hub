...
export async function POST(request: Request) {
  ...
  // Eliminar invitaciones pendientes anteriores para el mismo email
  await admin.from('invitations').delete().eq('email', email).is('used_at', null)
  ...
  try {
    ...
    if (emailError) {
      ...
      return NextResponse.json({ error: `Resend: ${emailError.message}` }, { status: 500 })
    }

    emailId = emailData?.id
  } catch (err: unknown) {
    await admin.from('invitations').delete().eq('token', token)
    const msg = err instanceof Error ? err.message : 'Error desconocido de Resend'
    console.error('[invite/send] Resend threw:', err)
